...
- 2026-07-01 — Creek now hugs the slope: replaced the per-segment flat water tiles (which floated over the hillside and left gaps) with a single continuous ribbon (`buildStreamRibbon`) whose cross-sections sit at the interpolated bed height — verified a uniform ~0.29 m gap above the carved channel down the gully and across the valley.
- 2026-07-01 — Pond + creek polish: (1) fixed the creek water quads standing vertical — `buildStreamSegment` basis was left-handed (`UP×forward`); now `forward×UP` so ribbons lie flat. (2) Rebuilt the rectangular "pool" into a NATURAL pond — organic wobbly outline, sunken bowl carved in the terrain (`pondBasin` in `ground.js`, `POND_DEPTH`), shoreline boulders + reeds + lily pads + a mossy focal stone, no stone rim/fountain. (3) Added a little arched wooden footbridge over the creek (`buildBridge` in `waterways.js`). Creek now spills into the pond at its west shore.
- 2026-07-01 — Waterfall now springs from the hills (fix): the creek is born high on the WEST valley wall (`creek.js` `CREEK_NODES`), runs down a carved gully, and the little waterfall (~1.5 m) steps over the hillside at its foot — no more artificial mound on the flat. Softer, wider valley walls (`BASIN_RADIUS`/`HILL_RADIUS`/`HILL_HEIGHT`, gentler `HILL_ROLL`), wider pad aprons; mist patches moved to the new waterfall/creek. Beds verified to sit below the natural hillside banks the whole way to the pool.
- 2026-07-01 — Shambhala terrain: `ground.js` now a valley/hill height field with a canonical `getTerrainHeight(x,z)`; flat valley floor cradled by soft green hills (opens toward the south approach). Buildings sit on level pads (`setTerrainPads` flattens each footprint + apron blend), player floor + planting + props + rocks all re-anchor to the terrain. New `creek.js` (carved channel + hillside waterfall) and `waterways.js` (animated creek ribbon, waterfall sheet, plunge foam, mist, bank stones) feeding the reflecting pool; shared water material via `waterFeature.createWaterMaterial`. Ground-hugging mist in the lows + fog tuning in `atmosphere.js`; animations drive the new water/mist. Pads keep interiors at `floorHeight` (< stair-snap), so buildings/windows/gates stay walkable.
